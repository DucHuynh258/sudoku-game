...
class ClientGUI:
    ...
    def connect_to_server(self):
        ...
        if not port_str.isdigit():
            messagebox.showerror("Lỗi", "Port phải là số từ 1 đến 65535")
            return
        port_val = int(port_str)
        if not (1 <= port_val <= 65535):
            messagebox.showerror("Lỗi", "Port phải trong khoảng 1-65535")
            return
        valid_ip = True
        if ip_str.lower() != "localhost":
            try:
                ...
                valid_ip = False
        if not valid_ip:
            messagebox.showerror("Lỗi", "IP không hợp lệ. Ví dụ: 127.0.0.1")
            return
